...
if [ ! $CC ]; then
	if which gcc-9 2>/dev/null >/dev/null ; then
		export CC=gcc-9
	else
		export CC=gcc
	fi
...
$CC -lm -O3 -fopenmp C-CODE/swiftstat7_cli.c -I $TEMP_DIR -o $BUILD_DIR/${BASENAME}p $* 2>> $CCOUT 1>> $CCOUT
if [ $? != 0 ]; then
...
$CC -lm -O3 -DDISABLE_THREADS C-CODE/swiftstat7_cli.c -I $TEMP_DIR -o $BUILD_DIR/$BASENAME $* 2>> $CCOUT >> $CCOUT
if [ $? != 0 ]; then
...
if [ $DO_MPI = 1 ]; then
	( $REXEC CMD SHLIB -fopenmp -static -D SWIFT_MPI -I $TEMP_DIR C-CODE/swiftstat7_r.c -o $BUILD_DIR/r/${BASENAME}_r.so --clean --preclean 2>> $CCOUT >> $CCOUT && cp $BUILD_DIR/r/* $RINSTALL_DIR 2>> $CCOUT >> $CCOUT ; ) &&
	( $REXEC CMD SHLIB -static -D SWIFT_MPI -D DISABLE_THREADS -I $TEMP_DIR C-CODE/swiftstat7_r.c -o $BUILD_DIR/r/${BASENAME}_r_s.so --clean --preclean 2>> $CCOUT >> $CCOUT && cp $BUILD_DIR/r/* $RINSTALL_DIR 2>> $CCOUT >> $CCOUT ; )
else
	( $REXEC CMD SHLIB -fopenmp -static -I $TEMP_DIR C-CODE/swiftstat7_r.c -o $BUILD_DIR/r/${BASENAME}_r.so --clean --preclean 2>> $CCOUT >> $CCOUT && cp $BUILD_DIR/r/* $RINSTALL_DIR 2>> $CCOUT >> $CCOUT ; ) &&
	( $REXEC CMD SHLIB -static -D DISABLE_THREADS -I $TEMP_DIR C-CODE/swiftstat7_r.c -o $BUILD_DIR/r/${BASENAME}_r_s.so --clean --preclean 2>> $CCOUT >> $CCOUT && cp $BUILD_DIR/r/* $RINSTALL_DIR 2>> $CCOUT >> $CCOUT ; )
fi
if [ $? != 0 ]; then
...
if [ $DO_MPI = 1 ] ; then
	...
	$CC -I $TEMP_DIR -fopenmp -lm -O3 C-CODE/swiftstat7_mpi.c -o $BUILD_DIR/${BASENAME}mpi -D SWIFT_MPI_MAIN $* 2>> $CCOUT >> $CCOUT
	if [ $? != 0 ]; then
