...
double lower_inc_log_gamma_reg(double a, double x) {
   /* Check parameters */
   if(x==0.0) return(-INFINITY);
   if((x<0.0) || (a<=0.0)) stop(1, "lower_inc_log_gamma_reg(%lf, %lf) is an invalid call", a, x);
...
double upper_inc_log_gamma_reg(double a, double x) {
   /* Check parameters */
   if((x<0.0) || (a<=0.0)) stop(1, "upper_inc_log_gamma_reg(%lf, %lf) is an invalid call", a, x);
...
double gammalogcdf(double x, double a, double b) {
	...
	//printf("l %.2lf u %.2lf\n", lower_inc_log_gamma_reg(a, x * b), lower_inc_gamma_reg(a, x * b));
		return lower_inc_log_gamma_reg(a, x * b);}
}
