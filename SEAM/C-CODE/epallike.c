...
double loggausslike(double *d,int N,double val) {
	double sum,sqs,sd,bandwidth,P_sum;
	int i;
	sum = d[1];
	sqs = sq(d[1]);
	for ( i=2; i<=N; i++ )  {
	...
	/* Scott's rule */ 
	bandwidth = 3.49*sd/pow(N,1.0/3.0);
	double * P = vector(double, N);
	for( i=1; i<=N; i++ ) {
		P[i] = gaussloglike(val, d[i], bandwidth);
	}
	double ret = logsumexp(P, N) - log(N);
