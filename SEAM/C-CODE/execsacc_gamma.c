    ...
    if ( fitting <= 0 ) {
        ...
        (*k) = l;
            ...
            if(refix) {
                ...
                if(x1 > x0 + 0.5) {
                    dx = x1 - x0;
                    saccerr = logsubexp(gammalogcdf(dx+0.5, a_r, b_r), gammalogcdf(dx-0.5, a_r, b_r)) - gammalogcdf(ux_r, a_r, b_r) + log1p(-relative_position);
                    //printf("dx=%.2lf, a_r=%.2lf, b_r=%.2lf, ux_r=%.2lf\n", dx, a_r, b_r, ux_r);
                } else if(x1 < x0 - 0.5) {
                    dx = x0 - x1;
                    saccerr = logsubexp(gammalogcdf(dx+0.5, a_l, b_l), gammalogcdf(dx-0.5, a_l, b_l)) - gammalogcdf(ux_l, a_l, b_l) + log(relative_position);
                    //printf("dx=%.2lf, a_l=%.2lf, b_l=%.2lf, ux_l=%.2lf\n", dx, a_l, b_l, ux_l);
                } else {
                    //printf("dx=%.2lf, a_r=%.2lf, b_r=%.2lf, ux_r=%.2lf, a_l=%.2lf, b_l=%.2lf, ux_l=%.2lf\n", dx, a_r, b_r, ux_r, a_l, b_l, ux_l);
                ...
                dx = x1 - x0;
                saccerr = logsubexp(gammalogcdf(dx+0.5, a_r, b_r), gammalogcdf(dx-0.5, a_r, b_r)) - gammalogcdf(ux_r, a_r, b_r);
                //printf("dx=%.2lf, a_r=%.2lf, b_r=%.2lf, ux_r=%.2lf\n", dx, a_r, b_r, ux_r);
            } else if(bwsacc) {
                dx = x0 - x1;
                saccerr = logsubexp(gammalogcdf(dx+0.5, a_l, b_l), gammalogcdf(dx-0.5, a_l, b_l)) - gammalogcdf(ux_l, a_l, b_l);
                //printf("dx=%.2lf, a_l=%.2lf, b_l=%.2lf, ux_l=%.2lf\n", dx, a_l, b_l, ux_l);
            }

            if(verbose) printf("%d:%.2lf->%d:%.2lf | %.1lf->%.1lf (r=%.2lf) : Pspat = %.2lf\n", *k, x0, *next_tar, upcoming_letter_pos, x0, x1, relative_position, saccerr);
